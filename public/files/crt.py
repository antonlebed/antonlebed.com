...
def eigenvalue(residues, ring, weights=None):
    """Eigenvalue of the channel-step graph (one +-1 step in one channel;
    the Cayley graph generated by the atomic idempotents, i.e. the
    product of cycles C_q1 x ... x C_qk) at the CRT-coordinate character
    of this tuple: lambda_n = sum_i w_i * cos(2*pi*r_i / q_i).
    Default weight = 2 = adjacency; eigenvalue(0) = 2k is the degree.
    (explore_eigenvalue_operator.py)"""
    if weights is None:
        weights = [2] * ring.k
    return sum(
        w * _cos_2pi(r, q)
