...
via the Chebotarev density theorem. Adjoining 2-power roots of
ALPHA over the cyclotomic base costs full degree at every level,
and the reason is the norm: N(alpha) = -1 is negative, hence not a
square in the real field Q(sqrt D), so Q(sqrt alpha)/Q is not
normal (Halter-Koch's criterion) - and a non-normal quartic sits
inside no abelian extension of Q, which every Q(sqrt D, zeta_{2^s})
is. The theta-tower rides one step behind (theta^{1/2} = i alpha,
free once i is present), and the one further collapse is classical
and cyclotomic - sqrt 2 inside Q(zeta_8) at D = 2 - moving only the
joint distribution of (split, v2(p-1)): that shift is the whole
Pell correction, and the free-half conditional it leaves untouched.
