...
1. EVERY COLLISION THE (3,4) SWEEP FINDS AT t = 12 DESCENDS TO AN EDGE,
   AND AT ALMOST ALL OF THEM THAT WAS MEASURED RATHER THAN FORCED
   (observation, 336 instances in one box; the (2,6) half of t = 12 is
   unswept and outside this claim). The (3,4) sweep at t = 12
   found 336 non-unique products over 85,253 pairs. Every one reads
   ...
   one-dimensional, where delta <= dim forces the answer. So the reading
...
2. THE FRONT WAS PRICED 4.4x HIGH, AND THE OVERPRICING IS THE SEED COUNT
   ...
   grading -- runs in 245.3 s, just over four minutes, inside the
   ...
   the half-million an 18-minute quote at 2.1 ms implies. A price
   inherited across a question's change of vocabulary was never re-derived
   against the filter that actually bounds the walk.
...
RUN RECORD (this file, under memwatch.py at the 512MB default, 245.8 s
wall, peak working set 76.6 MB). The diagnostic half (S0 and S1, under
...
def stage2(pairs, cores, gensof):
    ...
    for i, (A, B) in enumerate(pairs):
        ...
        if n > 1:
            dim, d, w, b = descent_sampled(facs[0], facs[1], g)
